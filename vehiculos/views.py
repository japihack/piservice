...
class VehiculoOrdenReparacionView(CreateView):
	...
	def get_context_data(self, **kwargs):
		...
		contador_or = Reparacion.objects.count()
		#contador_or += 1
		contador_or = contador_or + 1
		context['contador_or'] = contador_or
		#self.object = self.get_object
		#self.object.cod_reparacion = contador_or
	...
	def post(self, request, *args, **kwargs):
		...
		if form.is_valid():
			...
			orden.cod_reparacion=contador_or
			orden.save()
			return HttpResponseRedirect(self.get_success_url())
		else:
			return self.render_to_response(self.get_context_data(form=form))

#def orden_reparacion_add(request):
	# mirar en myshop -> cart.py el modelo cart(object) y en el libro django2 by example páginia 421
